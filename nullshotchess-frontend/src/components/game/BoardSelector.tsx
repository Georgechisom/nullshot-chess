...
const BoardSelector = ({ selected, onSelect }: BoardSelectorProps) => {
  const boards: { value: BoardDesign; label: string; light: string; dark: string }[] = [
    { value: 'walnut', label: 'Dark Walnut', light: '#C19A6B', dark: '#654321' },
    { value: 'oak', label: 'Light Oak', light: '#F5DEB3', dark: '#D2B48C' },
    { value: 'marble', label: 'Black Marble', light: '#E8E8E8', dark: '#1A1A1A' },
    { value: 'jade', label: 'Green Jade', light: '#A8E6CF', dark: '#00563F' },
    { value: 'lapis', label: 'Blue Lapis', light: '#87CEEB', dark: '#191970' },
    { value: 'mahogany', label: 'Rich Mahogany', light: '#CD5C5C', dark: '#420420' },
    ...
    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
      {boards.map((board) => (
        ...
        >
          ...
          <div className="w-full h-20 rounded-lg mb-3 grid grid-cols-4 grid-rows-4 overflow-hidden">
            {Array.from({ length: 16 }).map((_, i) => {
              ...
              return (
                <div
                  key={i}
                  style={{ backgroundColor: isLight ? board.light : board.dark }}
                />
              );
            })}
          </div>
