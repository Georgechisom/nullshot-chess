  ...
  const modes = [
    {
      ...
      description: 'Local multiplayer - Play against a friend on the same device',
      ...
      {modes.map((mode) => (
        ...
        >
          <div className="flex items-start gap-4">
            <div className={`${selected === mode.value ? 'text-accent' : 'text-muted-foreground'}`}>
              {mode.icon}
            </div>
            <div className="flex-1">
              <h3 className="text-xl font-semibold mb-2">{mode.label}</h3>
